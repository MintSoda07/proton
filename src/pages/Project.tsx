...
import ModelingViewPro from "../editor/ModelingViewPro";
...
export default function Project() {
  ...
  // ---- modeling은 분리된 Editor(모듈화된 세 페이지 중 1개)로 풀스크린 구성
  if (type === "modeling") {
    return (
      <FullscreenProjectView
        ...
        rightArea={
          <div className="flex items-center gap-2">
            {/* TODO: 아래 버튼들은 EditorContext와 이벤트로 연결하거나 props로 콜백을 넘겨 바인딩하세요 */}
  ...
  const tools: ToolItem[] = [
    { id: "move", name: "이동", icon: <MoveIcon className="w-5 h-5" />, shortcut: "V", category: "basic" },
    { id: "brush", name: "브러쉬", icon: <BrushIcon className="w-5 h-5" />, shortcut: "B", category: "paint" },
    { id: "zoom", name: "확대/축소", icon: <ZoomIcon className="w-5 h-5" />, shortcut: "Z", category: "basic" },
    { id: "layers", name: "레이어", icon: <LayersIcon className="w-5 h-5" />, shortcut: "L", category: "organize" },
  ];

  const getStatusBadge = (status: ProjectInfo["status"]) => {
    const styles = {
    ...
    <div className="h-screen bg-base text-white flex flex-col overflow-hidden">
      ...
      <div className="bg-black/30 border-b border-white/10 px-4 py-2 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate("/proton")} className="text-white/70 hover:text-white transition-colors">
            ← 돌아가기
          </button>
          <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-xs text-white/60">마지막 저장: {projectInfo.lastModified}</div>
        </div>
      </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`px-3 py-1 rounded text-sm transition-colors ${
                  showLayers ? "bg-white/20 text-white" : "bg-white/10 text-white/70"
                }`}
              >
                레이어
              </button>
              <button
                ...
                className={`px-3 py-1 rounded text-sm transition-colors ${
                  showProperties ? "bg-white/20 text-white" : "bg-white/10 text-white/70"
                }`}
              >
                속성
              </button>
          ...
          <div className="flex-1 relative overflow-hidden">
            ...
            >
              <div className="text-center text-white/60">
                ...
                <p className="text-sm">
                  {projectType === "sketch" && "스케치 캔버스가 여기에 표시됩니다"}
                  {projectType === "drawing" && "드로잉 캔버스가 여기에 표시됩니다"}
                  {projectType === "sculpt" && "3D 스컬핑 뷰포트가 여기에 표시됩니다"}
                  {projectType === "modeling" && "3D 모델링 뷰포트가 여기에 표시됩니다"}
                  {!["sketch", "drawing", "sculpt", "modeling"].includes(projectType) && "작업 캔버스가 여기에 표시됩니다"}
                </p>
                ...
                {["배경", "스케치", "색칠", "오버레이"].map((layer, i) => (
                  <div
                    ...
                    className={`p-3 rounded-lg border cursor-pointer transition-colors ${
                      i === 1 ? "border-white/30 bg-white/10" : "border-white/10 bg-white/5 hover:bg-white/8"
                    }`}
                  >
            ...
            <div className="flex-1 p-4">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="range" min="1" max="100" defaultValue="25" className="w-full accent-white/60" />
                  <div className="text-xs text-white/60 mt-1">25px</div>
                </div>

                <div>
                  <label className="block text-sm font-medium mb-2">불투명도</label>
                  <input type="range" min="0" max="100" defaultValue="100" className="w-full accent-white/60" />
                  <div className="text-xs text-white/60 mt-1">100%</div>
                </div>

                <div>
                  <label className="block text-sm font-medium mb-2">경도</label>
                  <input type="range" min="0" max="100" defaultValue="50" className="w-full accent-white/60" />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-6 gap-2">
                    {["#000000", "#FFFFFF", "#FF0000", "#00FF00", "#0000FF", "#FFFF00"].map((color) => (
                      <div key={color} className="w-8 h-8 rounded-lg border border-white/20 cursor-pointer" style={{ backgroundColor: color }} />
                    ))}
                  </div>
                </div>
              </div>
